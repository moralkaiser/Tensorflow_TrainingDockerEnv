...
function makeStopTrainingCurlToTrainingContainer($userhandle,$gpu){
        $url = "http://tf_trainingcontainer_" .$userhandle .  ":801/stoptraining.php?gpu=" . $gpu;
        $handle = curl_init();
...
function makeStopJupyterCurlToTrainingContainer($userhandle,$gpu){
...
foreach ($userList as $userhandle)
{
	$userdata = getUserData($userhandle);
	$gpu = $userdata[0];
	$status = $userdata[2];

	if($status == "training")
	{
        	makeStopTrainingCurlToTrainingContainer($userhandle,$gpu);
	}

	if($status == "jupyter")
	{
        	makeStopJupyterCurlToTrainingContainer($userhandle,$gpu);
	}
